...
// Guards the "Check out our GitHub" announcement the same way the Discord,
// X, and Instagram ones above are guarded — every user (including guests)
// gets it filed into their notification centre exactly once. Versioned the
// same way: bump GITHUB_NOTIFICATION_VERSION in lib/data.ts whenever the
// repo link (or the announcement itself) changes, so everyone with the old
// entry gets it swapped for a fresh one instead of a stale link.
const GITHUB_NOTIFIED_PREFIX = 'gh:notifications:github-sent:'

export function hasSentGithubNotification(userKey: string, version: string): boolean {
  if (typeof window === 'undefined') return false
  try {
    return window.localStorage.getItem(`${GITHUB_NOTIFIED_PREFIX}${userKey}`) === version
  } catch {
    return false
  }
}

export function markGithubNotificationSent(userKey: string, version: string) {
  if (typeof window === 'undefined') return
  try {
    window.localStorage.setItem(`${GITHUB_NOTIFIED_PREFIX}${userKey}`, version)
  } catch {
    // Best-effort — worst case it's filed again on a later visit.
  }
}

// Guards the single "all 4 social channels are live" milestone notice —
// separate from (and in addition to) the four individual Discord/X/
// Instagram/GitHub notices above. Same mechanism: every user (including
// guests) gets it filed into their Notification Centre exactly once per
// version.
const ALL_SOCIALS_LIVE_NOTIFIED_PREFIX = 'gh:notifications:all-socials-live-sent:'

export function hasSentAllSocialsLiveNotification(userKey: string, version: string): boolean {
  if (typeof window === 'undefined') return false
  try {
    return window.localStorage.getItem(`${ALL_SOCIALS_LIVE_NOTIFIED_PREFIX}${userKey}`) === version
  } catch {
    return false
  }
}

export function markAllSocialsLiveNotificationSent(userKey: string, version: string) {
  if (typeof window === 'undefined') return
  try {
    window.localStorage.setItem(`${ALL_SOCIALS_LIVE_NOTIFIED_PREFIX}${userKey}`, version)
  } catch {
    // Best-effort — worst case it's filed again on a later visit.
  }
}
