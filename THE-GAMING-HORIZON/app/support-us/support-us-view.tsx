...
import { getSupportWall, type SupportWallEntry } from '@/lib/support-contributions'
...
const WALL_TIER_ICONS: Record<string, LucideIcon> = { backer: Shield, supporter: Award, champion: Gem, legend: Crown }
...
const TRUST_POINTS: { icon: LucideIcon; text: string }[] = [
  ...
  {
    icon: Lock,
    text: 'No card or bank details are ever collected — only the amount and the UPI transaction reference you submit afterwards, for manual verification.',
  },
  {
    icon: Info,
    text: 'Every payment is checked by hand against the project\u2019s own UPI/bank statement before a badge is granted — nothing is auto-approved.',
  },
  {
    icon: CheckCircle2,
...
export function SupportUsView() {
  ...
  const [wall, setWall] = useState<SupportWallEntry[]>([])
  const [wallLoaded, setWallLoaded] = useState(false)

  useEffect(() => {
    let cancelled = false
    getSupportWall().then((rows) => {
      if (!cancelled) {
        setWall(rows)
        setWallLoaded(true)
      }
  ...
  return (
    <main id="support-us-page" className="relative min-h-screen pb-24">
      ...
      {/* Founding supporters goal / Supporters Wall */}
      <section id="wall" className="relative scroll-mt-32 px-4 py-16 cq-py-24">
          ...
          <Reveal>
            <div className="rounded-3xl border border-dashed border-border/70 bg-background/40 px-6 py-10 text-center">
              ...
              <p className="mx-auto mt-2 max-w-md text-sm leading-relaxed text-muted-foreground">
                {wallLoaded && wall.length > 0
                  ? "That's the goal before the Public Beta ships. Every verified supporter's badge and name appears below."
                  : "That's the goal before the Public Beta ships. The Supporters Wall is empty for now — be one of the first names on it."}
              </p>
              <div className="mx-auto mt-5 h-1.5 max-w-xs overflow-hidden rounded-full bg-[rgb(var(--accent-1)/0.14)]">
                <div
                  className="h-full rounded-full bg-[rgb(var(--accent-1))] transition-[width] duration-700"
                  style={{ width: `${Math.min(100, (wall.length / FOUNDING_SUPPORTER_GOAL) * 100)}%` }}
                />
              </div>
              <p className="mt-2 text-[11px] font-medium text-muted-foreground">
                {wall.length} of {FOUNDING_SUPPORTER_GOAL} claimed
              </p>

              {wallLoaded && wall.length > 0 ? (
                <div className="mx-auto mt-6 flex max-w-2xl flex-wrap items-center justify-center gap-2.5">
                  {wall.slice(0, 40).map((entry) => {
                    const Icon = WALL_TIER_ICONS[entry.tierId] ?? Shield
                    return (
                      <div
                        key={entry.id}
                        title={entry.displayName}
                        className="flex items-center gap-2 rounded-full border border-border/70 bg-background/60 py-1.5 pl-1.5 pr-3.5"
                      >
                        <span className="grid size-7 shrink-0 place-items-center rounded-full bg-[rgb(var(--accent-1)/0.14)] text-[10px] font-bold text-[rgb(var(--accent-1))]">
                          {initialsFrom(entry.displayName)}
                        </span>
                        <span className="text-xs font-medium text-foreground">{entry.displayName}</span>
                        <Icon className="size-3.5 text-[rgb(var(--accent-1))]" />
                      </div>
                    )
                  })}
                  {wall.length > 40 && (
                    <span className="text-xs text-muted-foreground">+{wall.length - 40} more</span>
                  )}
                </div>
              ) : (
