...
import { Award, Crown, Gem, Heart, QrCode, Rocket, Server, Shield, ShieldCheck, Sparkles, type LucideIcon } from 'lucide-react'
...
import { FOUNDING_SUPPORTER_GOAL, MIN_SUPPORT_AMOUNT_INR, QUICK_SUPPORT_AMOUNTS, WHY_SUPPORT } from '@/lib/support-us'

const WHY_ICONS = [Server, Rocket, ShieldCheck]
const TIER_PREVIEW_ICONS = [Shield, Award, Gem, Crown]
const TIER_ICONS: Record<string, LucideIcon> = { backer: Shield, supporter: Award, champion: Gem, legend: Crown }
    ...
    <section id="support-us" className="relative px-4 py-24 cq-py-32">
      <div className="mx-auto max-w-6xl">
        ...
        <Reveal delay={0.1} className="mt-10">
          <div id="wall" className="scroll-mt-32 rounded-3xl border border-dashed border-border/70 bg-background/40 px-6 py-8 text-center">
            {!wallLoaded || wall.length === 0 ? (
              <div className="mx-auto flex max-w-md flex-col items-center">
                ...
                <p className="mt-4 text-sm font-semibold text-foreground">
                  {wallLoaded ? 'The Supporters Wall is empty — for now' : 'Loading the Supporters Wall…'}
                </p>
                <p className="mt-1 max-w-sm text-xs leading-relaxed text-muted-foreground">
                  Every verified supporter&apos;s badge and name is featured right here. Be the first to claim a spot.
                </p>
              </div>
            ) : (
              <div className="mx-auto max-w-3xl">
                <p className="text-sm font-semibold text-foreground">
                  {wall.length} of {FOUNDING_SUPPORTER_GOAL} Founding Supporters
                </p>
                <div className="mt-5 flex flex-wrap items-center justify-center gap-3">
                  {wall.slice(0, 24).map((entry) => {
                    const Icon = TIER_ICONS[entry.tierId] ?? Shield
                    return (
                      <div
                        key={entry.id}
                ...
                {wall.length > 24 && (
                  <p className="mt-3 text-xs text-muted-foreground">+{wall.length - 24} more supporters</p>
                )}
              </div>
            )}
          </div>
        ...
        <Reveal delay={0.12} className="mt-8">
          <div className="mx-auto flex max-w-2xl flex-col items-center gap-3 text-center">
            ...
            <p className="text-xs leading-relaxed text-muted-foreground">
              Pay by scanning the QR in your own UPI app — no cards, no accounts. No payment details are ever
              collected on this website; only the UPI transaction reference you submit afterwards is stored, for
              manual verification.
            </p>
            <button
              type="button"
