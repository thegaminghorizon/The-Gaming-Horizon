...
import {
  ...
  GITHUB_URL,
  GITHUB_NOTIFICATION_VERSION,
  ALL_SOCIALS_LIVE_NOTIFICATION_VERSION,
} from '@/lib/data'
...
const GITHUB_NOTIFICATION_TITLE = 'Check out our GitHub'
const ALL_SOCIALS_LIVE_NOTIFICATION_TITLE = 'All our social channels are live'
...
export function NotificationsProvider({ children }: { children: ReactNode }) {
  ...
  // Same one-time treatment for the "check out our GitHub" announcement —
  // guarded and versioned the same way as the Discord, X, and Instagram
  // notices above.
  useEffect(() => {
    if (hasSentGithubNotification(userKey, GITHUB_NOTIFICATION_VERSION)) return
    removeNotificationsByTitle(userKey, GITHUB_NOTIFICATION_TITLE)
    addNotification(userKey, {
      title: GITHUB_NOTIFICATION_TITLE,
      body: 'Our GitHub repository is public — browse the source, follow progress between releases, and see how Gaming Horizon is actually being built.',
      icon: 'celebration',
      actionUrl: GITHUB_URL,
      actionLabel: 'View GitHub',
    })
    markGithubNotificationSent(userKey, GITHUB_NOTIFICATION_VERSION)
    refresh()
  }, [refresh, userKey])

  // A single milestone notice — every user (signed in or guest) gets this
  // exactly once, on top of (not instead of) the four individual
  // Discord/X/Instagram/GitHub notices above, now that all four platforms
  // are confirmed live together. No single actionUrl here since it covers
  // four different destinations — the individual notices above (and the
  // footer) already carry a real link for each one.
  useEffect(() => {
    if (hasSentAllSocialsLiveNotification(userKey, ALL_SOCIALS_LIVE_NOTIFICATION_VERSION)) return
    removeNotificationsByTitle(userKey, ALL_SOCIALS_LIVE_NOTIFICATION_TITLE)
    addNotification(userKey, {
      title: ALL_SOCIALS_LIVE_NOTIFICATION_TITLE,
      body: "Discord, X, Instagram, and GitHub are all live now — find quick links to each in this Notification Centre, or in the footer anytime.",
      icon: 'celebration',
    })
    markAllSocialsLiveNotificationSent(userKey, ALL_SOCIALS_LIVE_NOTIFICATION_VERSION)
    refresh()
  }, [refresh, userKey])
